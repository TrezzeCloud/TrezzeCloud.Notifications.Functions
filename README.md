...
- integração da aplicação com Datadog por meio de `Datadog.AzureFunctions` e da Serverless Compatibility Layer. O Bicep não cria uma conta Datadog: credenciais e configurações do ambiente permanecem externas.
...
A Function está instrumentada para Datadog por meio de `Datadog.AzureFunctions` e da inicialização da Serverless Compatibility Layer. Application Insights permanece opcional. O envio efetivo de telemetria depende das credenciais e configurações mantidas no ambiente externo; nenhuma chave Datadog é versionada. Permanecem como ações externas a publicação da imagem, a implantação no Azure, a conectividade com RabbitMQ e a configuração das permissões e dos recursos externos.
